...
function Graph({ caseData, activeStepId, finalizedStepIds, isPlaying, onSelectStep, onShowDetails }: Props) {
  ...
  const nodes: ClinicalFlowNode[] = useMemo(() => caseData.nodes.map(step => ({ id: step.stepId, type: "clinical", width: 212, height: 90, position: { x: step.position.x / 280 * 260, y: step.position.y / 210 * 156 }, data: { step, active: step.stepId === activeStepId, completed: finalizedStepIds.includes(step.stepId), playing: isPlaying }, ariaLabel: `${step.stepId}：${step.title}。${zh(step.actionType)}`, className: finalizedStepIds.includes(step.stepId) || step.stepId === activeStepId ? "revealed-node" : "future-node" })), [caseData, activeStepId, finalizedStepIds, isPlaying]);
  ...
  return <section className="trajectory-card" aria-label="临床决策轨迹">
    <div className="graph-heading"><h2>决策轨迹</h2><div><button className="text-control" onClick={onShowDetails}>步骤详情</button><button className={"text-control " + (overview ? "control-active" : "")} onClick={() => { setOverview(value => !value); if (!overview) void flow.fitView({ padding: 0.08, duration }); }}>{overview ? "跟随当前步骤" : "全局视图"}</button></div></div>
    <div className="trajectory-canvas" data-testid="trajectory-graph">
      <ReactFlow<ClinicalFlowNode, ClinicalFlowEdge> nodes={nodes} edges={edges} nodeTypes={nodeTypes} edgeTypes={edgeTypes} onNodeClick={(_, node) => onSelectStep(node.id)} nodesDraggable={false} nodesConnectable={false} minZoom={0.12} maxZoom={1.5} defaultViewport={{ x: 35, y: 30, zoom: 1 }} attributionPosition="bottom-right" preventScrolling={false} fitView={false} ariaLabelConfig={FLOW_LABELS} aria-label="可交互的临床决策轨迹图" />
      <div className="graph-tools"><Button variant="ghost" size="icon" aria-label="缩小轨迹图" onClick={() => void flow.zoomOut({ duration })}><Minus size={14} /></Button><Button variant="ghost" size="icon" aria-label="放大轨迹图" onClick={() => void flow.zoomIn({ duration })}><Plus size={14} /></Button><Button variant="ghost" size="icon" aria-label="适应轨迹图" onClick={() => { setOverview(true); void flow.fitView({ padding: 0.08, duration }); }}><Maximize size={13} /></Button></div>
    </div>
    <div className="graph-legend"><span><i />继续</span><span><i className="consult" />会诊</span><span><i className="transfer" />转交主管</span><span>分支与回归主线请看箭头方向</span></div>
  </section>;
}
