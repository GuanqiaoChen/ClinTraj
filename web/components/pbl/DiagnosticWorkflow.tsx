...
import { Background, BackgroundVariant, BaseEdge, getSmoothStepPath, Handle, MarkerType, MiniMap, Position, ReactFlow, ReactFlowProvider, ViewportPortal, useReactFlow, useStore, type Edge, type EdgeProps, type Node, type NodeProps } from "@xyflow/react";
...
type GraphNode = Node<{ item: WorkflowNode; selected: boolean; muted: boolean; fresh: boolean; shared: number; onAdd: (id: string) => void }, "diagnosis">;
type GraphEdge = Edge<{ detourX?: number }, "path">;
type AddKind = "hypothesis" | "test" | "consultation";
const labels: Record<WorkflowNodeKind, string> = { hypothesis: "假设", test: "检查", evidence: "证据", consultation: "会诊", conclusion: "诊断确认" };
...
function DiagnosisEdge(props: EdgeProps<GraphEdge>) {
  ...
  let [path, labelX, labelY] = getSmoothStepPath({ sourceX: sx, sourceY: sy, targetX: tx, targetY: ty, sourcePosition: Position.Bottom, targetPosition: Position.Top, borderRadius: 16 });
  if (data?.detourX !== undefined) {
    const lane = data.detourX, exit = sy + 26, entry = ty - 26;
    ...
    labelX = lane; labelY = (exit + entry) / 2;
  }
  return <BaseEdge id={props.id} path={path} markerEnd={props.markerEnd} style={props.style} label={props.label} labelX={labelX} labelY={labelY} labelStyle={props.labelStyle} labelBgStyle={props.labelBgStyle} labelBgPadding={[7, 4]} labelBgBorderRadius={4} />;
}
...
function Composer({ nodes, parentId, onClose, onAdd }: { nodes: WorkflowNode[]; parentId: string | null; onClose: () => void; onAdd: (kind: AddKind, title: string, purpose: string, parents: string[]) => void }) {
  ...
  return <div className="dw-modal-layer" onClick={onClose}><form className="dw-composer" data-testid="diagnostic-composer" role="dialog" aria-modal="true" aria-labelledby={headingId} onClick={event => event.stopPropagation()} onSubmit={submit} onKeyDown={event => {
    ...
    <div className="dw-composer-types">{(["hypothesis", "test", "consultation"] as const).map(value => { const Icon = icons[value]; return <button type="button" key={value} aria-pressed={kind === value} onClick={() => setKind(value)}><Icon size={17} />{value === "hypothesis" ? "诊断假设" : value === "test" ? "检查决策" : "专科会诊"}</button>; })}</div>
...
function WorkflowCanvas() {
  ...
  const changed = stageIndex === 0 ? [] : allNodes.filter(node => node.introducedAt === stageIndex);
  ...
  const nodes: GraphNode[] = useMemo(() => view.nodes.map(item => ({ id: item.id, type: "diagnosis", position: { x: item.x, y: item.y }, width: VIEW_NODE_WIDTH, height: VIEW_NODE_HEIGHT,
    data: { item, selected: item.id === selectedId, muted: relatedIds !== null && !relatedIds.has(item.id), fresh: stageIndex > 0 && item.introducedAt === stageIndex, shared: allEdges.filter(edge => edge.target === item.id && allNodes.find(node => node.id === edge.source)?.kind === "hypothesis").length, onAdd: openComposer },
    ariaLabel: `${labels[item.kind]}：${item.title}，${item.statusLabel}`, ariaRole: "button", selected: item.id === selectedId,
  })), [view.nodes, selectedId, relatedIds, stageIndex, allEdges, allNodes, openComposer]);
  const edges: GraphEdge[] = useMemo(() => view.edges.map(edge => {
    ...
    const obstacles = view.nodes.filter(node => node.y > source.y && node.y < target.y && node.x + VIEW_NODE_WIDTH >= Math.min(source.x, target.x) && node.x <= Math.max(source.x, target.x) + VIEW_NODE_WIDTH);
    const color = edge.status === "ruled-out" ? "#c1c5ca" : edge.status === "active" ? "#7892ab" : "#9aa9b5";
    return { ...edge, label: undefined, type: "path", data: { detourX: obstacles.length ? Math.max(source.x, target.x, ...obstacles.map(node => node.x)) + VIEW_NODE_WIDTH + 30 : undefined }, animated: false,
      markerEnd: { type: MarkerType.ArrowClosed, width: 16, height: 16, color }, style: { stroke: color, strokeWidth: edge.status === "active" ? 1.8 : 1.5, strokeDasharray: edge.status === "ruled-out" ? "4 5" : undefined, opacity: relatedIds && !(relatedIds.has(edge.source) && relatedIds.has(edge.target)) ? 0.2 : 1 },
      labelStyle: { fill: "#697888", fontSize: 11, fontWeight: 500 }, labelBgStyle: { fill: "#f5f7f8" },
    };
  }), [view, relatedIds]);

  const fit = useCallback(() => void flow.fitView({ includeHiddenNodes: true, padding: { top: "102px", bottom: "68px", left: "32px", right: "32px" }, minZoom: 0.12, maxZoom: 1, duration }), [flow, duration]);
  useEffect(() => {
  ...
  function goTo(index: number) { setStageIndex(index); setPlayRequested(false); setSelectedId(null); setComposing(false); setFollowing(true); setAnnouncement(""); }
  function select(id: string) { setSelectedId(id); setPlayRequested(false); }
  ...
  return <section ref={panel} className={`dw-workflow${expanded ? " is-expanded" : ""}${collapsed ? " is-collapsed" : ""}`} aria-label="动态诊断工作流" style={{ "--dw-body-height": `${bodyHeight}px` } as CSSProperties} onKeyDown={event => {
    ...
    {!collapsed && <><div className="dw-body">
      <aside className="dw-hypotheses" aria-label="诊断假设列表"><div className="dw-rail-heading"><h3>诊断假设</h3><span>{String(hypotheses.length).padStart(2, "0")}</span></div><div className="dw-hypothesis-list">{hypotheses.map((hypothesis, index) => <button key={hypothesis.id} className={`dw-hypothesis dw-status-${hypothesis.status}${selectedId === hypothesis.id ? " is-selected" : ""}`} aria-label={`聚焦假设：${hypothesis.title}`} aria-pressed={selectedId === hypothesis.id} onClick={() => select(hypothesis.id)}><span className="dw-hypothesis-number">{hypothesis.status === "ruled-out" ? <X size={12} /> : hypothesis.status === "confirmed" ? <Check size={12} /> : String(index + 1).padStart(2, "0")}</span><span><strong>{hypothesis.title}</strong><small><i />{hypothesis.statusLabel}</small></span><ChevronRight size={13} /></button>)}</div>
        <button className="dw-add-button" aria-label="追加节点" onClick={() => openComposer()}><Plus size={16} />添加假设 / 决策</button>
        <div className="dw-rail-context"><span className="dw-overline">{recommended ? "优先验证" : ended ? "诊断已确认" : "本轮进展"}</span>{recommended ? <button onClick={() => select(recommended.id)}><ScanLine size={17} /><strong>{recommended.title}</strong><ArrowDownRight size={14} /></button> : <strong>{snapshot.stage.title}</strong>}<p>{recommended?.priority || snapshot.stage.summary}</p>{recommended && <span className="dw-burden">{recommended.burden}</span>}</div>
        <div className="dw-rail-footer"><i />合成研究病例</div>
      </aside>
      <div className="dw-canvas" data-testid="diagnostic-canvas">
        <div className="dw-canvas-heading"><span className="dw-stage-kicker">{String(stageIndex + 1).padStart(2, "0")}<span>/ 06</span></span><div><h3>{snapshot.stage.title}</h3><p>{stageIndex === 0 ? "3 个假设 · 优先复用检查" : changed.length ? `${changed.length} 个新节点 · ${snapshot.stage.label}` : snapshot.stage.label}</p></div>{!following && <button className="dw-recenter" aria-label="跟随进展" onClick={() => { setFollowing(true); fit(); }}><Crosshair size={14} />定位当前</button>}</div>
        <ReactFlow<GraphNode, GraphEdge> nodes={nodes} edges={edges} nodeTypes={nodeTypes} edgeTypes={edgeTypes} nodesDraggable={false} nodesConnectable={false} edgesFocusable={false} deleteKeyCode={null} minZoom={0.12} maxZoom={1.6} fitView fitViewOptions={{ includeHiddenNodes: true, padding: 0.15, maxZoom: 1 }} onNodeClick={(_, node) => select(node.id)} onPaneClick={() => setSelectedId(null)} onMoveStart={event => { if (event) setFollowing(false); }} ariaLabelConfig={FLOW_LABELS} aria-label="诊断验证画布" preventScrolling={false} onlyRenderVisibleElements>
          ...
          {mapVisible && <MiniMap<GraphNode> pannable zoomable style={{ width: 154, height: 92 }} nodeColor={node => node.data.item.status === "ruled-out" ? "#d2d5d8" : node.data.item.kind === "hypothesis" ? "#283335" : node.data.item.kind === "evidence" ? "#b9d3c8" : "#a3bed3"} maskColor="rgba(237,241,243,.7)" ariaLabel="工作流全局导航" />}
        </ReactFlow>
        ...
        {selected && <WorkflowInspector node={selected} stageIndex={stageIndex} related={allNodes.filter(node => node.id !== selected.id && allEdges.some(edge => (edge.source === selected.id && edge.target === node.id) || (edge.target === selected.id && edge.source === node.id)))} onClose={() => setSelectedId(null)} onSelect={select} onContinue={() => openComposer(selected.id)} />}
      </div>
    </div>
    ...
    <div className="dw-resize-bar"><span>{allNodes.length} 个节点<span>·</span>{allEdges.length} 条连接</span><div><Tool label="减小模块高度" disabled={expanded || bodyHeight <= 420} onClick={() => setBodyHeight(value => Math.max(420, value - 90))}><ChevronUp size={12} /></Tool><Tool label="增大模块高度" disabled={expanded || bodyHeight >= 870} onClick={() => setBodyHeight(value => Math.min(870, value + 90))}><ChevronDown size={12} /></Tool></div></div>
    </>}
