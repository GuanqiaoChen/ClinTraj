...
Clinician annotations are not required to build or exercise these technical components. They are required for clinical correctness, acceptable-alternative, and rationale-quality endpoints; the supplied workbook has no completed review rows. See [data inspection](data_inspection.md) for the source findings and limitations.
...
Run `npm run lint`, `npm run type-check`, `npm run test`, and `npm run build` from `web/`. See [demo documentation](demo.md) for controls, fixture provenance, the implemented architecture, and the future trace-source boundary.
...
Six baseline configurations and the executable component ablations live in `configs/experiments/`. The temporal-gate and clinician-intervention removal files are protocol declarations that fail explicitly in the physician-facing runner. See [reproducibility](reproducibility.md) and [experiment protocols](../experiments/README.md) before interpreting any run.

## Docker

The current Docker image starts the live API by default. Run the independent research smoke test explicitly:

```bash
docker build -t clintraj:0.1.0 .
docker run --rm clintraj:0.1.0 clintraj demo --decision reject --steps 1
```
...
docker run --rm \
  ...
  clintraj:0.1.0 clintraj experiment \
  --config multi_agent_graph_safety \
  --output experiments/runs
```
...
docker run --rm \
  ...
  clintraj:0.1.0 clintraj inspect /data/source.xlsx
```

## Repository guide

- [Architecture](architecture.md): method layers, data flow, agent contracts, safety boundary, runtime, and evaluation.
- [Interactive demo](demo.md): localhost setup, replay controls, five synthetic scenarios, and frontend integration boundaries.
- [Clinical graph semantics](clinical_graph_semantics.md): exact `CONTINUE`, `BRANCH`, `CONSULT`, `TRANSFER`, and `RETURN` invariants.
- [Data inspection](data_inspection.md): workbook structure, five selected cases, source/image discrepancies, and replay assumptions.
- [Reproducibility](reproducibility.md): environments, data integrity, experiment matrix, manifests, and known limits.
- [Methods draft](methods_draft.md), [novelty statement](novelty_statement.md), [reviewer red team](reviewer_red_team.md), and [readiness audit](top_journal_readiness_audit.md): publication-oriented method description and unresolved empirical requirements.
